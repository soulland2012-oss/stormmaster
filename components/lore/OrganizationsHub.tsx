...
import { organizations, type OrganizationSummary } from '@/data/organizations'
...
function OrgCard({ org }: { org: OrganizationSummary }) {
  return (
    <motion.div variants={itemVariants} className="group h-full">
      <Link href={org.href} className="block h-full">
        <div
        ...
        >
          ...
          <div
            ...
            style={{ boxShadow: `inset 0 0 0 1px ${org.accent}45` }}
          />
          ...
          <div className="relative z-10 p-7 h-full flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-3 mb-4">
                <span
                  ...
                  style={{ fontFamily: 'var(--font-cinzel, serif)', color: org.accent, border: `1px solid ${org.accent}50`, background: `${org.accent}15` }}
                >
                  {org.tag}
                </span>
              </div>
              <p className="font-cinzel text-[0.56rem] tracking-[0.2em] mb-1.5" style={{ fontFamily: 'var(--font-cinzel, serif)', color: `${org.accent}80` }}>
            ...
            <div>
              ...
              <div
                ...
                style={{ fontFamily: 'var(--font-cinzel, serif)', color: org.accent }}
              >
                ЧИТАТЬ СТАТЬЮ
...
function OrgGroup({ label, note, orgs }: { label: string; note?: string; orgs: OrganizationSummary[] }) {
  return (
    <div>
      <motion.div
      ...
      >
        ...
        <div>
          ...
          {note && <p className="text-stone-500 text-xs mt-1">{note}</p>}
        </div>
      </motion.div>
...
export default function OrganizationsHub() {
  const affiliated = organizations.filter((o) => o.affiliation)
  const independent = organizations.filter((o) => !o.affiliation)
    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            Дома, ордены и тайные культы, определяющие судьбы Калдериса — от благородных знамён Альянса до безымянных культов, не признающих ничьей власти.
          </motion.p>
        </motion.div>
      </div>
      ...
      <div style={{ background: 'rgb(246 245 250)' }}>
        <div className="max-w-7xl 2xl:max-w-[1900px] mx-auto px-6 lg:px-10 py-16 lg:py-20 space-y-20">
          <OrgGroup label="Альянс" note="Организации, присягнувшие людской державе Альянс." orgs={affiliated} />
          <OrgGroup label="Без аффилиации" note="Независимые культы, не подчиняющиеся ни одному государству." orgs={independent} />
        </div>
      </div>
    </div>
  )
